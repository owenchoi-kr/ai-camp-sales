...
메일·미팅·문서는 다 **상대와의 접점**이었다. 이번 소스는 결이 다르다 — **우리 팀 내부 논의**다. "이 계정 지원 이슈 있었지", "가격 어디까지 열어주기로 했지", "누가 이 계정 담당이었지". Slack 채널·스레드에 흩어진 이 맥락을 검색해 가져온다.
...
### Step 4: 지금 바로 실행해보기 (소스 4개 — 마지막 소스 하나 남음)

같은 담당 계정으로 한 번 더 돌려본다:
...
이제 4소스:
  ...
  + Notion(계정 상태·팩트)
  + Slack(우리 팀 내부 입장)
  → 여기에 Block 7에서 Salesforce(딜 현황)만 붙이면 360도가 완성된다
```

> 소스 1개일 때(Block 3)와 지금을 비교해보라. 같은 계정인데 브리핑의 밀도가 완전히 다르다. 이게 "만들면서 배운다"의 결과물이다. 마지막 조각 하나만 남았다.
...
스킬 진행 상황:
  ...
  소스 3: Notion      ✅
  소스 4: Slack       ✅ 채움!
  소스 5: Salesforce  [STUB]  ← Block 7에서 마지막으로 채움
```
...
AskUserQuestion({
  "questions": [{
    "question": "여러 소스 중 Slack(내부 논의)만 결이 다릅니다. 뭐가 다른가요?",
    "header": "Quiz 6",
    "options": [
...
정답: 1번. Gmail·Circleback·Notion은 상대와의 접점이고, Slack은 **우리 팀이 이 계정을 어떻게 보는지**를 준다. 여기에 Salesforce(딜 현황)까지 다섯 조각이 다 모여야 계정을 360도로 본다.
