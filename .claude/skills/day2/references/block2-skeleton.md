...
입력:  계정명 하나 (회사명 또는 @도메인)
        ↓
       Gmail · Circleback · Notion · Slack · Salesforce 5소스에서 이 계정 접점 수집
        ↓
...
빈 화면에서 스킬을 짜는 게 아니다. 운영진이 만든 **템플릿(뼈대)**을 복사한다. 이 뼈대에는 소스 5개가 **STUB**(빈칸)으로 들어 있다. 오늘은 이 STUB을 블록마다 하나씩 채운다.

```
Block 2 (지금):  [STUB Gmail] [STUB Circleback] [STUB Notion] [STUB Slack] [STUB Salesforce] ← 빈 뼈대
Block 3:         [Gmail]      [STUB]            [STUB]        [STUB]        [STUB]             ← 소스1 채움
Block 4:         [Gmail]      [Circleback]      [STUB]        [STUB]        [STUB]             ← 소스2 채움
Block 5:         [Gmail]      [Circleback]      [Notion]      [STUB]        [STUB]             ← 소스3 채움
Block 6:         [Gmail]      [Circleback]      [Notion]      [Slack]       [STUB]             ← 소스4 채움
Block 7:         [Gmail]      [Circleback]      [Notion]      [Slack]       [Salesforce]       ← 소스5 채움
Block 8:         [완성!] 병렬 수집 → 실전 계정으로 히스토리 브리핑
```
...
구조:
  ...
  📌 소스 5: Salesforce  [STUB - Block 7에서 붙여넣기/연결]
  📌 실행 흐름           [STUB - Block 8에서 완성]
  📌 출력 포맷           [STUB - Block 8에서 완성]
...
AskUserQuestion({
  "questions": [{
    ...
    "options": [
      ...
      {"label": "5개 소스를 한 번에 다 연결하고 끝낸다", "description": "한 번에 붙이지 않는다 — 하나씩 붙이며 체감하는 게 핵심"}
    ],
    "multiSelect": false
  }]
})
...
정답: 2번. 뼈대(STUB 5개)를 복사해두고, Block 3~7에서 하나씩 채운다. 채울 때마다 실행해보는 게 오늘의 핵심 리듬이다.
