...
오늘 스킬을 만들려면 Gmail·미팅노트·Notion·Slack을 Claude에 연결해야 한다. 이걸 가능하게 하는 게 **MCP**다.
...
> 지금 이 셋을 외울 필요 없다. **"방법이 여러 가지구나"** 정도만 알고 가면 된다.
...
정답: 1번.
피드백: "맞습니다. MCP = 표준 연결 규격. 오늘은 이 규격으로 Gmail·Circleback·Notion·Slack을 하나씩 꽂으면서, 그 위에서 도는 내 스킬을 만듭니다."
